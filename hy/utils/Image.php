...
class Image {
     ...
     * @param int $pos 水印图片位置
     * +--------------------------------
     * 1、顶部居左 2、顶部居中 3、顶部居右
     ...
     * @return void
     */
    static public function water($source, $water, $savename=null, $pos=0) {
        //检查文件是否存在
     ...
     * @return void
     */
    static function thumb2($image, $thumbname, $type='', $maxWidth=200, $maxHeight=200, $interlace=true) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        ...
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 8, $i * 10 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);
        }
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        $code = Strings::randString($length, 4);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $codex = Strings::msubstr($code, $i, 1);
